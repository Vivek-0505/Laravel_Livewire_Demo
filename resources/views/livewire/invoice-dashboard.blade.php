    ...
    <div class="md:px-6">
        <div class="flex justify-between items-center flex-wrap max-sm:pb-5">
            ...
            <div class="flex items-center gap-1.5">
                ...
                {{-- <button type="button"
                    class="cursor-auto bg-primarycolor text-white text-sm max-sm:text-xs py-1 font-medium px-3 rounded-md flex gap-1 items-center">
                    <svg class="fill-white" aria-hidden="true"
                        class="SVGInline-svg SVGInline--cleaned-svg SVG-svg Icon-svg Icon--add-svg Button-icon-svg Icon-color-svg Icon-color--white-svg"
                        height="12" width="12" viewBox="0 0 16 16" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        <li>
            <button wire:click="setTab('{{ $key }}')"
                class="@if ($activeTab === $key) border-b-2 border-primarycolor text-primarycolor @else text-lightgray @endif px-2 pb-2 text-sm">
                {{ $label }}
            </button>
        </li>
    @endforeach
                    ...
                    <tr class="border-b border-bordercolor">
                        ...
                        <td class="px-4 py-2">
                            <span
                                ...
                                @if ($invoice['status'] === 'draft') bg-draftbg text-lightgray border border-bordercolor
                                @elseif ($invoice['status'] === 'outstanding') bg-outstandingbg text-outstandingtext
                                @elseif ($invoice['status'] === 'paid') bg-paidbg text-paidtext @endif">
                                {{ ucfirst($invoice['status']) }}
                            </span>
                        </td>
                        <td class="px-4 py-2 text-lightgray text-sm">{{ $invoice['invoice_number'] }}</td>
                        <td class="px-4 py-2 text-lightgray text-sm">{{ $invoice['customer']['email'] }}</td>
                        <td class="px-4 py-2 text-lightgray text-sm whitespace-nowrap">{{ $invoice['formatted_due_date'] }}</td>
                        <td class="px-4 py-2 text-lightgray text-sm whitespace-nowrap">{{ $invoice['formatted_created_at'] }}</td>
                        <td class=" justify-center flex items-center px-4 py-2 ">
                            <div x-data="{ open: false }" @click.away="open = false">
                                ...
                                <ul x-show="open"
                                    ...
                                    <li><a href="#" wire:click="delete({{ $invoice->id }})" onclick="return confirm('Are you sure you want to delete this invoice?')"
